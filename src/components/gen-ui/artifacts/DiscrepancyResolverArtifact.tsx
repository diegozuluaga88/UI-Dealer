...
export default function DiscrepancyResolverArtifact({ issues, onResolve, onClose, title }: DiscrepancyResolverProps) {
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            ...
            case 'rule': return <Scale className="w-5 h-5 text-ai" />;
    ...
    return (
        <div className="relative w-full max-w-2xl bg-white dark:bg-zinc-800 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800 flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in duration-300 my-auto">
            ...
            {/* Header */}
            <div className={`shrink-0 p-6 border-b flex justify-between items-center z-10 ${isSubstitution ? 'bg-ai-light/50 dark:bg-ai/10 border-ai/10 dark:border-ai/30/50' : 'bg-white dark:bg-zinc-800 border-zinc-100 dark:border-zinc-800'}`}>
                <div>
                    <h2 className="text-xl font-bold font-brand flex items-center gap-2 text-foreground">
                        {isSubstitution ? <Sparkles className="w-6 h-6 text-ai" /> : getTypeIcon(currentIssue.type)}
                        {title || 'Review Discrepancy'}
                    </h2>
                    <p className="text-sm text-muted-foreground mt-1">
                        Issue {progressIndex} of {initialTotal} • <span className={isSubstitution ? "text-ai font-medium" : "text-amber-600 font-medium"}>Resolution Required</span>
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    <div className="w-32 h-2 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                        <div
                            className={`h-full transition-all duration-300 ${isSubstitution ? 'bg-ai' : 'bg-amber-500'}`}
            ...
            <div className="flex-1 overflow-y-auto p-8 scrollbar-micro bg-zinc-50/50 dark:bg-black/10">
                ...
                {isAutoFixing ? (
                    <div className="h-full flex flex-col items-center justify-center text-center py-12">
                        <Sparkles className="w-12 h-12 text-ai animate-pulse mb-4" />
                    ...
                    <div className="grid grid-cols-2 gap-8 h-full">
                        ...
                        {/* AI Suggestion */}
                        <div className="space-y-4">
                            <div className="text-xs font-bold text-ai uppercase tracking-wider flex items-center gap-2">
                                <Sparkles className="w-4 h-4" /> {currentIssue.metadata?.options ? 'Manual Substitution Required' : 'AI Recommendation'}
                                <span className="ml-auto text-[10px] bg-ai-light dark:bg-ai/10 text-ai px-2 py-0.5 rounded-full border border-ai/20 dark:border-ai/30">
                                    {confidence}% Confidence
                                </span>
                            </div>
                            <div className="bg-gradient-to-br from-ai-light to-white dark:from-ai/5 dark:to-zinc-900 p-6 rounded-xl border border-ai/20 dark:border-ai/30 shadow-sm h-full relative overflow-hidden flex flex-col">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-ai/5 rounded-full blur-2xl -mr-16 -mt-16 pointer-events-none"></div>

                                <div className="flex-1 space-y-4 relative z-10">
                                    <div>
                                        <label className="text-xs text-ai/70 dark:text-ai/70">Analysis</label>
                                        <div className="text-sm font-medium text-ai dark:text-ai-light mt-2 space-y-2">
                                            {currentIssue.suggestion.reason.split(',').map((criteria, idx) => (
                                                <div key={idx} className="flex items-start gap-2">
                                                    <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0 text-ai" />
                                    ...
                                    <div className="pt-4 border-t border-ai/10 dark:border-ai/30">
                                        <label className="text-xs text-ai/70 dark:text-ai/70">Proposed {currentIssue.suggestion.label}</label>
                                        ...
                                        {currentIssue.metadata?.options ? (
                                            ...
                                            <>
                                                <div className="font-mono text-sm bg-white dark:bg-zinc-800 p-2 rounded border border-ai/10 dark:border-ai/30 text-ai shadow-sm flex items-center gap-2 mt-1">
                ...
                <div className="flex flex-wrap sm:flex-nowrap items-center justify-center sm:justify-end gap-2 w-full sm:w-auto">
                    {issues.length > 0 && (
                        <button
                            ...
                            className={`mr-2 px-3 py-2 text-xs font-bold uppercase tracking-wide disabled:opacity-50 whitespace-nowrap flex items-center gap-1 rounded-lg transition-colors ${isSubstitution ? 'text-ai hover:bg-ai-light dark:hover:bg-ai/10' : 'text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/30'}`}
                        >
                    ...
                    <button
                        ...
                        disabled={isAutoFixing}
                        className={`flex-1 sm:flex-none px-6 py-2 rounded-lg text-white font-bold transition-colors shadow-sm flex items-center justify-center gap-2 whitespace-nowrap ${isSubstitution ? 'bg-ai hover:bg-ai shadow-ai/20 dark:shadow-ai/20' : 'bg-amber-600 hover:bg-amber-700 shadow-amber-200 dark:shadow-amber-900/20'}`}
                    >
