import { CheckBadgeIcon, ArrowDownTrayIcon, ShareIcon, ArrowRightIcon } from '@heroicons/react/24/outline';
...
export default function SuccessModal({ isOpen, poNumber = 'PO-2026-001', onClose }: SuccessModalProps) {
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/90 dark:bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-md border border-zinc-200 dark:border-zinc-800 text-center p-8 relative overflow-hidden">
                ...
                <div className="flex justify-center mb-6">
                    <div className="w-20 h-20 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center animate-in zoom-in spin-in-12 duration-500">
                ...
                <p className="text-muted-foreground mb-6">
                    Purchase Order <span className="font-mono font-medium text-foreground">{poNumber}</span> has been successfully created and sent for processing.
                </p>

                <div className="flex flex-col gap-3">
                    <button onClick={onClose} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-3 rounded-xl font-medium shadow-sm transition-transform active:scale-[0.98] flex items-center justify-center gap-2">
                        Return to Dashboard
