...
import { X, ArrowRight, ArrowLeft, CheckCircle2, XCircle, AlertTriangle, Sparkles, FileSearch, ClipboardCheck, FileText } from 'lucide-react'
...
const statusIcon = (s: 'match' | 'mismatch' | 'partial') => {
    if (s === 'match') return <CheckCircle2 className="w-5 h-5 text-green-500 dark:text-green-400" />;
    if (s === 'mismatch') return <XCircle className="w-5 h-5 text-red-500 dark:text-red-400" />;
            ...
            <Dialog as="div" className="relative z-[200]" onClose={onClose}>
                ...
                <div className="fixed inset-0 z-10 w-screen overflow-y-auto scrollbar-minimal">
                    <div className="flex min-h-full items-center justify-center p-4">
                        ...
                        >
                            <DialogPanel className="relative transform overflow-hidden rounded-2xl bg-background text-left shadow-2xl transition-all border border-border w-full sm:max-w-4xl">
                                {/* ──── Step 1: Select PO/ACK Pair ──── */}
                                {step === 'select' && (
                                    <div className="p-6 sm:p-8">
                                        <div className="flex justify-between items-start mb-6">
                                            <div>
                                                <DialogTitle as="h3" className="text-xl font-brand font-bold text-foreground mb-1 flex items-center gap-2.5">
                                                    ...
                                                    >
                                                        ...
                                                        {disc.autoFixSuggestion && (
                                                            <div className="p-2.5 rounded-lg bg-indigo-50/50 dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/10 mb-3 flex items-start gap-2">
                                                                <Sparkles className="w-3.5 h-3.5 text-indigo-500 dark:text-indigo-400 mt-0.5 shrink-0" />
